...
function calcularOtimizacao() {
    ...
    // Otimização adicional para combinar perfis inteiros em sobras maiores
    barras.forEach(barra => {
        if (barra.espacoLivre > 0) {
            // Tenta encontrar perfis inteiros que caibam na sobra
            const perfisInteirosCabiveis = perfisRestantes.filter(perfil => perfil.tamanho <= barra.espacoLivre);
            if (perfisInteirosCabiveis.length > 0) {
                // Encontra o perfil inteiro que melhor se encaixa na sobra
                const perfilMelhorEncaixe = perfisInteirosCabiveis.reduce((melhor, perfil) => {
                    return barra.espacoLivre - perfil.tamanho < barra.espacoLivre - melhor.tamanho ? perfil : melhor;
                }, perfisInteirosCabiveis[0]);

                // Adiciona o perfil inteiro à barra
                barra.perfis.push(perfilMelhorEncaixe);
                barra.espacoLivre -= perfilMelhorEncaixe.tamanho;

                // Remove o perfil inteiro da lista perfisRestantes
